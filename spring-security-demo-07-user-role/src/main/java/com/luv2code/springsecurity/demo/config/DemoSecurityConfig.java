...
//	}
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		  ...
		  auth.inMemoryAuthentication().withUser("prem").password(passwordEncoder().encode("test123")).roles("EMPLOYEE","MANAGER");
		  auth.inMemoryAuthentication().withUser("raj").password(passwordEncoder().encode("test123")).roles("EMPLOYEE","ADMIN");
